...
class pdf_Z():
    ...
    def crearPDF(self):
        #global c,dosColas, colaIzquierda,pmean,alpha,zc1,zc2,zp,pvalor

        w, h = letter

        def pruebaDosColas(c, pmean, alpha, zc1, zc2, zp, pvalor):
            ...
            c.drawImage("imagenes/formulaZ.png", 100, h-230, width=75, height=50)
            ...
            c.drawString(50, h-95, '     Zp = {:.2f}'.format(zp))
            ...
            if((zp >= zc1) & (zp <= zc2) & (pvalor > alpha)):
                ...
                c.drawString(50, h-425, 
                             '     b)El valor de "Z de prueba" Zp = {:.2f} se encuentra en el "área de no rechazo" dentro '.format(zp))
                c.drawString(50, h-440,'        del rango [{:.2f},{:.2f}].'.format(zc1,zc2))
                ...
                c.drawString(50, h-425, 
                             '     b)El valor de "Z de prueba" Zp = {:.2f} se encuentra en el "área de rechazo" fuera '.format(zp))
                c.drawString(50, h-440,'        del rango [{:.2f},{:.2f}].'.format(zc1,zc2))
                c.drawString(50, h-455, 
        ...
        def pruebaColaIzq(c, pmean, alpha, zc1, zp, pvalor):
            c.drawString(50, h-50, "PRUEBA DE HIPÓTESIS")
            c.drawString(50, h-80, "1.Formulación de hipótesis.")
            c.drawString(50, h-95, '   Ho: \u03BC \u2265 {:.2f}'.format(pmean))
            c.drawString(50, h-110, '   H1: \u03BC < {:.2f}'.format(pmean))
            ...
            if((zp >= zc1) & (pvalor > alpha)):
                c.drawString(50, h-410,'     a)No se rechaza Ho.')
                c.drawString(50, h-425, 
                             '     b)El valor de “Z de prueba ” Zp = {:.2f} se encuentra en el “área de no rechazo” a la derecha '.format(zp))
                c.drawString(50, h-440,'        de “Z crítico” Zc = {:.2f}.'.format(zc1))
                c.drawString(50, h-455, 
                             '     c)El pvalor es mayor que la significancia: pvalor = {:.4f} > \u03B1 = {:.4f}.'.format(pvalor,alpha))
            else:
                c.drawString(50, h-410,'     a)Se rechaza Ho.')
                c.drawString(50, h-425, 
                             '     b)El valor de “Z de prueba ” Zp = {:.2f} se encuentra en el “área de rechazo” a la izquierda '.format(zp))
                c.drawString(50, h-440,'        de “Z crítico” Zc = {:.2f}.'.format(zc1))
                c.drawString(50, h-455, 
                             '     c)El pvalor es menor que la significancia: pvalor = {:.4f} < \u03B1 = {:.4f}.'.format(pvalor,alpha))
        ...
        def pruebaColaDer(c, pmean, alpha, zc2, zp, pvalor):
            c.drawString(50, h-50, "PRUEBA DE HIPÓTESIS")
            c.drawString(50, h-80, "1.Formulación de hipótesis.")
            c.drawString(50, h-95, '   Ho: \u03BC \u2264 {:.2f}'.format(pmean))
            c.drawString(50, h-110, '   H1: \u03BC > {:.2f}'.format(pmean))
            ...
            if((zp <= zc2) & (pvalor > alpha)):
                c.drawString(50, h-410,'     a)No se rechaza Ho.')
                c.drawString(50, h-425, 
                             '     b)El valor de “Z de prueba ” Zp = {:.2f} se encuentra en el “área de no rechazo” a la izquierda '.format(zp))
                c.drawString(50, h-440,'        de “Z crítico” Zc = {:.2f}.'.format(zc2))
                c.drawString(50, h-455, 
                             '     c)El pvalor es mayor que la significancia: pvalor = {:.4f} > \u03B1 = {:.4f}.'.format(pvalor,alpha))
            else:
                c.drawString(50, h-410,'     a)Se rechaza Ho.')
                c.drawString(50, h-425, 
                             '     b)El valor de “Z de prueba ” Zp = {:.2f} se encuentra en el “área de rechazo” a la derecha '.format(zp))
                c.drawString(50, h-440,'        de “Z crítico” Zc = {:.2f}.'.format(zc2))
                c.drawString(50, h-455, 
                             '     c)El pvalor es menor que la significancia: pvalor = {:.4f} < \u03B1 = {:.4f}.'.format(pvalor,alpha))
        ...
        if(self.dosColas):
            pruebaDosColas(self.c,self.pmean,self.alpha,self.zc1,self.zc2,self.zp,self.pvalor)
        elif(self.colaIzquierda):
            pruebaColaIzq(self.c,self.pmean,self.alpha,self.zc1,self.zp,self.pvalor)
        else:
            pruebaColaDer(self.c,self.pmean,self.alpha,self.zc2,self.zp,self.pvalor)
